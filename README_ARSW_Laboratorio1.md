...
> En este laboratorio implementé la suite de pruebas automatizadas que valida la equivalencia funcional entre las cinco estrategias (`Sequential`, `FixedPool` (2/4/8) y `Virtual Threads`), utilizando tanto la latencia simulada como sin ella. Cada test compara los `matchingProviderIds` y el conteo de los proveedores consultados contra la línea de base secuencial, además verifico la ausencia de duplicados, el orden ascendente y la validación de los argumentos inválidos. Aprendí a balancear los tamaños de los datos de pruebas (20 vs 100 proveedores) debido a que esto aumenta el tiempo en el que se ejecutan las pruebas, porque cada proveedor adicional incrementa el tiempo, se ve cuando se hace `Thread.sleep()` en `isBlacklisted`, entonces cambiar de 20 a 100 proveedores en el test de equivalencia con I/O se reduce el tiempo de la prueba sin reducir la capacidad de detectar un bug de correción, porque esa capacidad que se cambia no depende la escala para este ejercicio.
...
|------------------|-----------------|---|---|
| Tomas Olaya Diaz | iAxstral        | Extended BenchmarkRunner to support SEQUENTIAL, FIXED and VIRTUAL strategies with warmups, equivalence validation and CSV output; executed the 10 mandatory benchmark configurations and documented results and environment. | 31a328b add my conclusion of the lab<br>21798ba add benchmark results and environment document<br>3ea7dd9 feat: implement BenchmarkRunner, equivalence validation and CSV output. |
...
| Claude (Anthropic) | Support for understanding Java 21 concurrency concepts (platform threads, virtual threads, race conditions, `ExecutorService`/`Future`) needed to design the automated equivalence test suite (Task 3). | - Explained the difference between platform threads and virtual threads, and how the existing implementation avoids race conditions.<br>- Clarified what "automated tests" means versus manually running the benchmark, to guide the test design decisions. | Ran `mvn clean test` locally and confirmed all tests pass; reviewed test coverage against the 10-point checklist in Section 10 before committing. | Test scope, provider counts (20 vs. 100 for the I/O scenario), and assertion targets were checked against the Section 10 checklist before acceptance. |
| Claude (Anthropic) | Implementation support for the concurrent search strategies (`FixedPoolBlackListSearch` and `VirtualThreadBlackListSearch`) and explanation of concurrency concepts (executors, futures, virtual threads). | - Requested an implementation of both classes following the constraints in Sections 7-8 of this README.<br>- Asked for a line-by-line explanation of the resulting code (executor lifecycle, result consolidation, interruption handling, why virtual threads help under blocking I/O). | Compiled the project and ran `mvn clean test`; manually verified the three strategies produced identical matches against the sequential baseline, with and without simulated I/O, before committing. | Accepted the implementation after confirming it satisfied every requirement in Sections 7 and 8, and after being able to explain the design decisions unprompted. |
Requirements:
